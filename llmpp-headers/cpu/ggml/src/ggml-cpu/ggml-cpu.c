...
// Single-thread build: disable OpenMP and threadpool threading
#undef  GGML_USE_OPENMP
#define GGML_SINGLE_THREAD
...
// Per-thread state (only thread 0 is used)
struct ggml_compute_state {
    struct ggml_threadpool * threadpool;
    int ith;
};

// No-op: single thread doesn't need polling
...
struct ggml_cplan ggml_graph_plan(
    ...
    // Single-thread build: always use 1 thread
    (void)n_threads;
    (void)threadpool;
    n_threads = 1;
...
struct ggml_threadpool * ggml_threadpool_new(struct ggml_threadpool_params * tpp) {
    ...
    ggml_threadpool_params_init(&params, 1);
    return ggml_threadpool_new_impl(&params, NULL, NULL);
}
...
enum ggml_status ggml_graph_compute(struct ggml_cgraph * cgraph, struct ggml_cplan * cplan) {
    ...
    if (threadpool == NULL) {
        ...
        ggml_threadpool_params_init(&ttp, 1);
    ...
    // Single-thread execution: just run thread 0 directly
    atomic_store_explicit(&threadpool->n_graph, 1, memory_order_relaxed);

    struct ggml_compute_state worker0;
    memset(&worker0, 0, sizeof(worker0));
    worker0.threadpool = threadpool;
    worker0.ith        = 0;

    ggml_graph_compute_thread(&worker0);
